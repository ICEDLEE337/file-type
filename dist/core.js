...
async function fileTypeFromStream(stream) {
    const strtok3 = await Promise.resolve().then(() => __importStar(require('strtok3/core'))); // eslint-disable-line n/file-extension-in-import
...
class FileTypeParser {
    ...
    async parse(tokenizer) {
        ...
        if (this.checkString('ID3')) {
            ...
            const id3HeaderLength = await tokenizer.readToken(util_js_1.uint32SyncSafeToken);
...
async function fileTypeStream(readableStream, { sampleSize = minimumBytes } = {}) {
    const { default: stream } = await Promise.resolve().then(() => __importStar(require('node:stream')));
        ...
        readableStream.once('readable', () => {
            (async () => {
                try {
                    ...
                    const chunk = readableStream.read(sampleSize) ?? readableStream.read() ?? node_buffer_1.Buffer.alloc(0);
                    try {
